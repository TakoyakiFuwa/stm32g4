...
/*  全局变量  */
//关于渲染队列，不用于接口，不需要使用
tftu_ui* UI_RENDER_QUEUE[200];	//UI渲染队列 200算是渲染缓冲区大小，
								//更改时应该同时更改UI_Init"创建渲染队列"中for循环的数量
uint16_t ui_render_index;		//渲染队列队尾(可添加渲染的位置)
tftu_ui	 null_ui_render_queue;	//用于表示UI渲染的队列尾，变相实现空指针

//光标
tftu_pointer UI_CURSOR;			//光标

//项目中的Page UI 字体
tftf_font FONT[10];				//字体
tftu_ui UI[10];					//UI
...
/**@brief  用于充当函数指针的空指针
  ...
  *		   会把这个函数塞到UI对象的接口上
  */
static void NULL_UI_Func(struct tftu_ui* none)
{
	QY_Printf("\r\n 警告！当前UI对象没有绑定该操作函数 \r\n");
}
...
void Init_UI(void)
{
	//UI创建接口
	//创建光标
//	FONT[0] = TFTF_CreateFont((const char*)font_ASCII_PIXEL_3216,32,16,COLOR_RED,COLOR_YELLOW);
//	UI_CURSOR.font = &FONT[0];
...
/**@brief  具体的渲染函数，用于向屏幕输出
  *@param  void
  *@retval void
  *@add    放在循环中或者定时器定时调用，tftu_ui.Func_Render_N只在这里和Page_Change中出现
  */
...
/**@brief  创捷页面
  ...
  *		   tftu_page中含有数组ui_index[100]，不可以用 数组=数组
  *		   而UI_CreateUI函数中并没有对tftu_ui的数组(notes[20]/value_text[20]进行操作)
...
void UI_ChangePage(tftu_page* new_page)
{
	...
	tftu_page* p = UI_CURSOR.ptr_page;
	...
	//绑定指针
	UI_CURSOR.ptr_ui = new_page->start_ui;
...
//下面进行测试
void Clock_Num_Render(tftu_ui* u)
{
	TFTF_ShowNum(u->x,u->y,u->value_num,*u->font,2);
}
void Clock_Num_Add1(tftu_ui* u)
{
	u->value_num += 1;
	UI_Cursor_Refresh();
}
void Clock_Num_Mul1(tftu_ui* u)
{
	u->value_num -= 1;
	UI_Cursor_Refresh();
}
void Clock_Left(tftu_ui* u)
{
